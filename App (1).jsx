...
// Extended to 36 months (extrapolated after M24)
const BASE_UNITS_36 = [0,19,49,92,135,190,233,276,331,378,438,510,564,622,679,741,801,872,936,1006,1077,1147,1218,1308,1398,1498,1608,1718,1838,1968,2108,2258,2418,2588,2768,2958];
const BASE_OPEX_36  = [55,48,48,52,52,55,57,57,60,60,63,65,68,68,72,72,75,78,80,80,85,85,88,90,93,96,99,102,105,108,112,116,120,124,128,132].map(v=>v*1000);
const MONTHS_36     = Array.from({length:36},(_,i)=>`M${i+1}`);
// Legacy aliases (24-month defaults)
const BASE_UNITS = BASE_UNITS_36.slice(0,24);
const BASE_OPEX  = BASE_OPEX_36.slice(0,24);
const MONTHS     = MONTHS_36.slice(0,24);
...
function buildBudget(investment, alloc, allocMeta) {
  const meta = allocMeta || DEFAULT_ALLOC_META;
  return meta.map(m => ({ name:m.label, key:m.key, pct:alloc[m.key]??0, value:Math.round(investment*(alloc[m.key]??0)/100), color:m.color }));
}

function changeAlloc(alloc, key, rawVal, allocMeta) {
  const meta     = allocMeta || DEFAULT_ALLOC_META;
  const clamped  = Math.max(1, Math.min(90, rawVal));
  const others   = meta.map(m => m.key).filter(k => k !== key);
...
function buildFinancials(investment, sellPrice, productCost, alloc, baseReinvest = 0.40, numMonths = 24) {
  const n          = Math.max(6, Math.min(36, numMonths));
  const monthsArr  = MONTHS_36.slice(0, n);
  const unitsArr   = BASE_UNITS_36.slice(0, n);
  const opexArr    = BASE_OPEX_36.slice(0, n);
  const scale      = Math.pow(investment / 1e6, 0.85);
  const opScale    = Math.pow(investment / 1e6, 0.50);
  const invBudget  = investment * (alloc.inventory??35) / 100;
  const setupTotal = investment * ((alloc.warehouse??15) + (alloc.tech??8) + (alloc.contingency??12)) / 100;
  ...
  return monthsArr.map((month, i) => {
    const units   = Math.round(unitsArr[i] * scale);
    const revenue = units * sellPrice;
    const cogs    = units * productCost;
    const opex    = Math.round(opexArr[i] * opScale);
...
// ─── BUDGET EDITOR (editable names, add/remove) ──────────────────────────────
const ALLOC_COLORS = [C.gold, C.mid, C.amber, C.rust, C.forest, "#F4A261", "#CE93D8", "#64B5F6", "#81C784", "#FF7043"];
let _allocColorIdx = 6; // next color to assign

function BudgetEditor({ investment, alloc, onAlloc, allocMeta, onAllocMeta }) {
  const meta   = allocMeta || DEFAULT_ALLOC_META;
  const budget = buildBudget(investment, alloc, meta);
  const total  = meta.reduce((s, m) => s + (alloc[m.key] || 0), 0);

  // Inline label editing
  const [editingLabel, setEditingLabel] = useState(null);
  const [labelDraft,   setLabelDraft]   = useState('');
  const labelRef = useRef(null);
  useEffect(() => { if (editingLabel !== null) labelRef.current?.focus(); }, [editingLabel]);

  const saveLabel = (key) => {
    const trimmed = labelDraft.trim();
    if (!trimmed) { setEditingLabel(null); return; }
    onAllocMeta(meta.map(m => m.key === key ? { ...m, label: trimmed } : m));
  ...
  const addCategory = () => {
    ...
    const color  = ALLOC_COLORS[_allocColorIdx % ALLOC_COLORS.length];
    _allocColorIdx++;
    const newMeta = [...meta, { key: newKey, label: 'New Category', color, min: 1, max: 50 }];
    // Take 5% from the largest category
    const sorted  = [...meta].sort((a, b) => (alloc[b.key] || 0) - (alloc[a.key] || 0));
    const donor   = sorted[0];
    const newAlloc = { ...alloc, [donor.key]: Math.max(1, (alloc[donor.key] || 0) - 5), [newKey]: 5 };
    onAllocMeta(newMeta);
    onAlloc(newAlloc);
  };
  ...
  const removeCategory = (key) => {
    if (meta.length <= 2) return; // keep at least 2
    const removedPct = alloc[key] || 0;
    const remaining  = meta.filter(m => m.key !== key);
    // Redistribute removed % proportionally to remaining
    const remSum = remaining.reduce((s, m) => s + (alloc[m.key] || 0), 0);
    const newAlloc = { ...alloc };
    delete newAlloc[key];
    if (remSum > 0) {
      remaining.forEach(m => {
        newAlloc[m.key] = parseFloat(((alloc[m.key] || 0) + removedPct * (alloc[m.key] || 0) / remSum).toFixed(1));
      });
    } else {
      const each = 100 / remaining.length;
      remaining.forEach(m => { newAlloc[m.key] = parseFloat(each.toFixed(1)); });
    }
    onAllocMeta(remaining);
    onAlloc(newAlloc);
  };

  const resetAll = () => {
    onAllocMeta(DEFAULT_ALLOC_META);
    onAlloc({ inventory:35, warehouse:15, tech:8, marketing:18, salaries:12, contingency:12 });
  };
    ...
    <div style={{ background:C.panel, border:`1px solid ${C.gold}22`, borderRadius:14, padding:24 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
        <div>
          ...
          <div style={{ color:C.muted, fontSize:11, fontFamily:mono, marginTop:3 }}>Edit names · Adjust sliders · Add or remove categories</div>
        </div>
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          ...
          <button onClick={addCategory} style={{ background:"transparent", border:`1px solid ${C.forest}66`, color:"#81C784", borderRadius:8, padding:"6px 12px", fontSize:11, fontFamily:mono, cursor:"pointer" }}>+ Add</button>
          <button onClick={resetAll}    style={{ background:"transparent", border:`1px solid ${C.gold}44`, color:C.gold, borderRadius:8, padding:"6px 14px", fontSize:11, fontFamily:mono, cursor:"pointer" }}>Reset</button>
        </div>
      </div>
      {/* Stacked bar */}
      <div style={{ display:"flex", height:14, borderRadius:7, overflow:"hidden", marginBottom:20, gap:1 }}>
        {budget.filter(b=>(b.pct||0)>0).map((b,i) => <div key={i} style={{ flex:b.pct||0, background:b.color, transition:"flex 0.25s" }} title={`${b.name}: ${(b.pct||0).toFixed(1)}%`} />)}
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(295px,1fr))", gap:14 }}>
        {meta.map(m => {
          const b    = budget.find(x=>x.key===m.key) || { pct:0, value:0 };
          const pct  = alloc[m.key] || 0;
          const isEditing = editingLabel === m.key;
          return (
            <div key={m.key} style={{ background:C.dark, border:`1px solid ${m.color}33`, borderRadius:10, padding:"14px 16px" }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8, flex:1, minWidth:0 }}>
                  ...
                  {isEditing ? (
                    <input ref={labelRef} value={labelDraft} onChange={e=>setLabelDraft(e.target.value)}
                      onBlur={()=>saveLabel(m.key)}
                      onKeyDown={e=>{ if(e.key==="Enter")saveLabel(m.key); if(e.key==="Escape"){setEditingLabel(null);} }}
                      style={{ background:C.panel, border:`1px solid ${m.color}`, borderRadius:5, color:C.cream, padding:"3px 8px", fontSize:12, fontFamily:sans, fontWeight:600, outline:"none", width:"100%" }} />
                  ) : (
                    <span style={{ color:C.cream, fontSize:13, fontWeight:600, cursor:"text", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}
                      onClick={()=>{ setEditingLabel(m.key); setLabelDraft(m.label); }}>
                      {m.label} <span style={{ color:m.color, fontSize:9, opacity:0.6 }}>✎</span>
                    </span>
                  )}
                </div>
                <div style={{ display:"flex", gap:6, alignItems:"center", flexShrink:0 }}>
                  <div style={{ display:"flex", alignItems:"center", background:C.panel, border:`1px solid ${m.color}55`, borderRadius:6, overflow:"hidden" }}>
                    <input type="number" min={m.min||1} max={m.max||90} step={0.5} value={parseFloat(pct.toFixed(1))}
                      onChange={e=>onAlloc(changeAlloc(alloc, m.key, Number(e.target.value), meta))}
                      style={{ background:"transparent", border:"none", outline:"none", color:m.color, padding:"4px 6px", fontSize:13, fontFamily:mono, fontWeight:700, width:50, textAlign:"right" }} />
                    <span style={{ color:m.color, fontSize:12, fontFamily:mono, paddingRight:6 }}>%</span>
                  </div>
                  <span style={{ color:C.amber, fontSize:12, fontFamily:mono, minWidth:60, textAlign:"right" }}>SAR {(b.value/1000).toFixed(0)}K</span>
                  {meta.length > 2 && (
                    <button onClick={()=>removeCategory(m.key)} title="Remove category"
                      style={{ background:"transparent", border:"none", color:"#F44336", cursor:"pointer", fontSize:14, padding:"0 2px", opacity:0.6, lineHeight:1 }}>✕</button>
                  )}
                </div>
              </div>
              <input type="range" min={m.min||1} max={m.max||90} step={0.5} value={pct}
                onChange={e=>onAlloc(changeAlloc(alloc, m.key, Number(e.target.value), meta))}
                style={{ accentColor:m.color, cursor:"pointer", width:"100%" }} />
              <div style={{ display:"flex", justifyContent:"space-between", color:C.muted, fontSize:9, fontFamily:mono, marginTop:3 }}>
                <span>{m.min||1}%</span><span>{m.max||90}%</span>
              </div>
            </div>
          );
        })}
...
// ─── TAB: OVERVIEW ────────────────────────────────────────────────────────────
function OverviewTab({ data, budget, inputs, alloc, onAlloc, allocMeta, onAllocMeta }) {
  const { investment, sellPrice, productCost } = inputs;
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:12, marginBottom:22 }}>
        ...
        <KPICard label={data.length>=24?"Year 2 Revenue":"Late Revenue"}    value={`SAR ${(y2/1e6).toFixed(2)}M`} sub="Projected" accent={C.rust} />
        <KPICard label={`${data.length}M Net Profit`}    value={`SAR ${(totalProfit/1000).toFixed(0)}K`} sub="Cumulative" accent={totalProfit>0?C.forest:"#F4A261"} />
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:18 }}>
        ...
        <div style={{ background:C.panel, border:`1px solid ${C.gold}22`, borderRadius:14, padding:22 }}>
          ...
          {[
            ...
            { k:"Est. Customer Acq. Cost",v:`SAR ${Math.round(mktBudget/Math.max(1,data[Math.min(11,data.length-1)].customers))}` },
          ].map((r,i)=>(
      ...
      <BudgetEditor investment={investment} alloc={alloc} onAlloc={onAlloc} allocMeta={allocMeta} onAllocMeta={onAllocMeta} />
    </div>
  );
}
...
// ─── TAB: TIMELINE (fully editable) ──────────────────────────────────────────
const DEFAULT_PHASES = [
  { id:1, phase:"Phase 1", name:"Foundation & Setup",  months:"M1–M2",   color:C.gold,   tasks:["Register company (CR + ZATCA + SASO)","Secure warehouse in Riyadh (600-800 sqm)","Sign investor term sheets","Tech stack selection (Shopify / custom)","Brand identity & naming","Payment gateway (Mada, STC Pay, Apple Pay)"] },
  { id:2, phase:"Phase 2", name:"Sourcing & Build",     months:"M2–M4",   color:C.forest, tasks:["Factory audits & sample orders from China","Negotiate MOQ & payment terms (30/70)","Website dev & product photography","WMS setup","Hire: Ops Manager, warehouse staff, Social Media Mgr","Soft launch on Instagram & Snapchat"] },
  { id:3, phase:"Phase 3", name:"Launch & Acquire",     months:"M4–M6",   color:C.rust,   tasks:["Official store launch","Influencer seeding campaign (Tier 1 + Tier 2)","Launch paid social (Meta, Snapchat)","First China shipment (FCL or LCL)","Customer service (WhatsApp + CRM)","KPI dashboard go-live"] },
  { id:4, phase:"Phase 4", name:"Scale & Optimize",     months:"M6–M12",  color:C.mid,    tasks:["Analyze top SKUs, cut underperformers","Expand influencer roster (20-30 creators)","Launch loyalty program","Optimize China freight","Explore second product line","Monthly investor reporting"] },
  { id:5, phase:"Phase 5", name:"Expansion",            months:"M12–M24", color:C.amber,  tasks:["Expand to Jeddah & Dammam","Launch B2B / corporate gifting channel","Add 2nd China supplier per category","Explore GCC expansion (Kuwait, UAE)","Automate reorder & demand forecasting","Series A / bridge funding round"] },
];
const PHASE_COLORS = [C.gold, C.forest, C.rust, C.mid, C.amber, "#CE93D8", "#64B5F6", "#81C784"];
let _phaseIdCounter = 6;

function TimelineTab({ numMonths, onNumMonths }) {
  const [phases,     setPhases]     = useState(DEFAULT_PHASES);
  const [open,       setOpen]       = useState(null);
  const [editingId,  setEditingId]  = useState(null); // which phase is being edited inline
  const [editDraft,  setEditDraft]  = useState({});
  const [taskEditing, setTaskEditing] = useState(null); // {phaseId, taskIdx}
  const [taskDraft,   setTaskDraft]   = useState('');

  // ── Phase-level CRUD ──────────────────────────────────────
  const updatePhase = (id, patch) => setPhases(ps => ps.map(p => p.id===id ? {...p,...patch} : p));

  const addPhase = () => {
    _phaseIdCounter++;
    const color = PHASE_COLORS[phases.length % PHASE_COLORS.length];
    const lastN = phases.length + 1;
    setPhases(ps => [...ps, { id:_phaseIdCounter, phase:`Phase ${lastN}`, name:"New Phase", months:`M${numMonths}-M${numMonths+2}`, color, tasks:["New task"] }]);
  };

  const removePhase = (id) => { if(phases.length<=1) return; setPhases(ps=>ps.filter(p=>p.id!==id)); if(open===id)setOpen(null); };

  const movePhase = (id, dir) => {
    setPhases(ps => {
      const idx = ps.findIndex(p=>p.id===id);
      const next = idx + dir;
      if (next < 0 || next >= ps.length) return ps;
      const arr = [...ps];
  ...
  const addTask = (phaseId) => {
    updatePhase(phaseId, { tasks: [...(phases.find(p=>p.id===phaseId)?.tasks||[]), "New task"] });
  };

  const removeTask = (phaseId, taskIdx) => {
    const p = phases.find(x=>x.id===phaseId);
    if (!p || p.tasks.length <= 1) return;
    updatePhase(phaseId, { tasks: p.tasks.filter((_,i)=>i!==taskIdx) });
  };

  const saveTask = (phaseId, taskIdx) => {
    const trimmed = taskDraft.trim();
    if (!trimmed) { setTaskEditing(null); return; }
    const p = phases.find(x=>x.id===phaseId);
    const tasks = [...p.tasks]; tasks[taskIdx] = trimmed;
    updatePhase(phaseId, { tasks });
    setTaskEditing(null);
  };

  // ── Phase header inline edit ──────────────────────────────
  const startEdit = (p) => { setEditingId(p.id); setEditDraft({ phase:p.phase, name:p.name, months:p.months }); };
  const saveEdit  = () => { if(editingId) updatePhase(editingId, editDraft); setEditingId(null); };

  const InputStyle = (accent=C.gold) => ({
    background: C.dark, border:`1px solid ${accent}55`, borderRadius:6,
    color:C.cream, padding:"4px 8px", fontSize:12, fontFamily:sans,
    fontWeight:600, outline:"none", width:"100%"
  });

  return (
    <div>
      <SectionTitle title="Project Timeline & Milestones" sub="Editable · Add/Remove Phases & Tasks · Reorder" />

      {/* Duration control */}
      <div style={{ background:C.panel, border:`1px solid ${C.gold}22`, borderRadius:12, padding:"14px 20px", marginBottom:18, display:"flex", gap:24, alignItems:"center", flexWrap:"wrap" }}>
        <div style={{ color:C.gold, fontSize:10, fontFamily:mono, letterSpacing:2 }}>SIMULATION DURATION</div>
        <div style={{ display:"flex", alignItems:"center", gap:12, flex:1 }}>
          <input type="range" min={6} max={36} step={1} value={numMonths}
            onChange={e=>onNumMonths(Number(e.target.value))}
            style={{ accentColor:C.gold, cursor:"pointer", flex:1, minWidth:140 }} />
          <div style={{ display:"flex", alignItems:"center", background:C.dark, border:`1px solid ${C.gold}44`, borderRadius:8, overflow:"hidden" }}>
            <input type="number" min={6} max={36} step={1} value={numMonths}
              onChange={e=>onNumMonths(Math.max(6,Math.min(36,Number(e.target.value))))}
              style={{ background:"transparent", border:"none", outline:"none", color:C.gold, padding:"6px 10px", fontSize:14, fontFamily:mono, fontWeight:700, width:56, textAlign:"center" }} />
            <span style={{ color:C.muted, padding:"6px 10px", fontSize:12, fontFamily:mono }}>months</span>
          </div>
        </div>
        <div style={{ display:"flex", gap:8 }}>
          {[6,12,18,24,36].map(n=>(
            <button key={n} onClick={()=>onNumMonths(n)} style={{ background:numMonths===n?C.gold:C.dark, color:numMonths===n?C.dark:C.muted, border:`1px solid ${numMonths===n?C.gold:C.mid}`, borderRadius:6, padding:"5px 10px", fontSize:11, fontFamily:mono, cursor:"pointer" }}>{n}M</button>
          ))}
        </div>
        <button onClick={addPhase} style={{ background:"transparent", border:`1px solid ${C.forest}66`, color:"#81C784", borderRadius:8, padding:"7px 14px", fontSize:11, fontFamily:mono, cursor:"pointer" }}>+ Add Phase</button>
      </div>

      {/* Phase pills */}
      <div style={{ display:"flex", gap:8, marginBottom:22, overflowX:"auto" }}>
        {phases.map((p,i)=>(
          <div key={p.id} onClick={()=>setOpen(open===p.id?null:p.id)} style={{ flex:1, minWidth:110, background:`${p.color}18`, border:`1px solid ${p.color}${open===p.id?"88":"33"}`, borderRadius:10, padding:"10px 14px", textAlign:"center", cursor:"pointer", transition:"all 0.2s" }}>
      ...
      {phases.map((p,i)=>(
        <div key={p.id} style={{ marginBottom:10, background:C.panel, border:`1px solid ${open===p.id?p.color+"66":C.mid}`, borderRadius:12, overflow:"hidden" }}>
          {/* Header */}
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 16px" }}>
            {editingId === p.id ? (
              // Editing mode
              <div style={{ display:"flex", gap:8, alignItems:"center", flex:1, flexWrap:"wrap" }} onClick={e=>e.stopPropagation()}>
                <input value={editDraft.phase} onChange={e=>setEditDraft(d=>({...d,phase:e.target.value}))}
                  style={{ ...InputStyle(p.color), width:90, color:p.color, fontSize:12 }} placeholder="Phase #" />
                <input value={editDraft.name} onChange={e=>setEditDraft(d=>({...d,name:e.target.value}))}
                  style={{ ...InputStyle(C.cream), width:180, fontSize:13 }} placeholder="Phase name" />
                <input value={editDraft.months} onChange={e=>setEditDraft(d=>({...d,months:e.target.value}))}
                  style={{ ...InputStyle(C.muted), width:110, color:C.muted, fontSize:12 }} placeholder="M1–M6" />
                <button onClick={saveEdit} style={{ background:p.color, color:C.dark, border:"none", borderRadius:6, padding:"5px 12px", fontSize:11, fontFamily:mono, fontWeight:700, cursor:"pointer" }}>Save</button>
                <button onClick={()=>setEditingId(null)} style={{ background:"transparent", border:`1px solid ${C.mid}`, color:C.muted, borderRadius:6, padding:"5px 10px", fontSize:11, fontFamily:mono, cursor:"pointer" }}>Cancel</button>
              </div>
            ) : (
              <div style={{ display:"flex", gap:12, alignItems:"center", flex:1, cursor:"pointer" }} onClick={()=>setOpen(open===p.id?null:p.id)}>
            ...
            <div style={{ display:"flex", gap:4, alignItems:"center", marginLeft:8 }}>
              {editingId !== p.id && (
                <>
                  <button onClick={e=>{e.stopPropagation();startEdit(p);}} title="Edit" style={{ background:"transparent", border:"none", color:C.muted, cursor:"pointer", fontSize:13, padding:"2px 5px" }}>✎</button>
                  <button onClick={e=>{e.stopPropagation();movePhase(p.id,-1);}} disabled={i===0} title="Move up" style={{ background:"transparent", border:"none", color:i===0?C.mid:C.muted, cursor:i===0?"default":"pointer", fontSize:13, padding:"2px 5px" }}>↑</button>
                  <button onClick={e=>{e.stopPropagation();movePhase(p.id,1);}} disabled={i===phases.length-1} title="Move down" style={{ background:"transparent", border:"none", color:i===phases.length-1?C.mid:C.muted, cursor:i===phases.length-1?"default":"pointer", fontSize:13, padding:"2px 5px" }}>↓</button>
                  <button onClick={e=>{e.stopPropagation();removePhase(p.id);}} title="Remove phase" disabled={phases.length<=1} style={{ background:"transparent", border:"none", color:phases.length<=1?C.mid:"#F44336", cursor:phases.length<=1?"default":"pointer", fontSize:14, padding:"2px 5px", opacity:phases.length<=1?0.3:0.7 }}>✕</button>
                </>
              )}
              <span onClick={()=>setOpen(open===p.id?null:p.id)} style={{ color:p.color, fontSize:18, cursor:"pointer", padding:"2px 5px" }}>{open===p.id?"-":"+"}</span>
            </div>
          </div>
          ...
          {open===p.id && (
            <div style={{ padding:"0 16px 16px" }}>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))", gap:10, marginBottom:10 }}>
                {p.tasks.map((t,j)=>(
                  <div key={j} style={{ display:"flex", gap:8, alignItems:"flex-start", padding:"10px 14px", background:C.dark, borderRadius:8, border:`1px solid ${C.mid}`, position:"relative" }}>
                    <span style={{ color:p.color, fontSize:12, marginTop:1, flexShrink:0 }}>◆</span>
                    {taskEditing?.phaseId===p.id && taskEditing?.taskIdx===j ? (
                      <div style={{ flex:1 }}>
                        <input value={taskDraft} onChange={e=>setTaskDraft(e.target.value)}
                          onBlur={()=>saveTask(p.id,j)}
                          onKeyDown={e=>{ if(e.key==="Enter")saveTask(p.id,j); if(e.key==="Escape")setTaskEditing(null); }}
                          autoFocus
                          style={{ background:C.panel, border:`1px solid ${p.color}`, borderRadius:5, color:C.cream, padding:"4px 8px", fontSize:12, fontFamily:sans, outline:"none", width:"100%" }} />
                      </div>
                    ) : (
                      <span style={{ color:C.sage, fontSize:13, flex:1, cursor:"text" }}
                        onClick={()=>{ setTaskEditing({phaseId:p.id,taskIdx:j}); setTaskDraft(t); }}>{t} <span style={{ color:p.color, fontSize:9, opacity:0.5 }}>✎</span></span>
                    )}
                    {p.tasks.length > 1 && (
                      <button onClick={()=>removeTask(p.id,j)} style={{ background:"transparent", border:"none", color:"#F44336", cursor:"pointer", fontSize:12, padding:0, opacity:0.5, flexShrink:0 }}>✕</button>
                    )}
                  </div>
                ))}
                {/* Add task card */}
                <div onClick={()=>addTask(p.id)} style={{ display:"flex", gap:8, alignItems:"center", justifyContent:"center", padding:"10px 14px", background:`${p.color}08`, borderRadius:8, border:`1px dashed ${p.color}44`, cursor:"pointer", minHeight:44 }}>
                  <span style={{ color:p.color, fontSize:20, lineHeight:1 }}>+</span>
                  <span style={{ color:p.color, fontSize:12, fontFamily:mono }}>Add Task</span>
                </div>
              </div>
            </div>
          )}
...
function FinancialTab({ data }) {
  ...
  const doExport = () => exportToCSV(`financial_model_${data.length}m.csv`, [
  ...
  return (
    <div>
      <SectionTitle title={`Financial Model — ${data.length} Months`} sub="All Numbers Recalculate Live" />
...
function ScenarioTab({ onLoadScenario }) {
  ...
  const scenarios = useMemo(() => PRESET_SCENARIOS.map(s => {
    const data = buildFinancials(s.inputs.investment, s.inputs.sellPrice, s.inputs.productCost, s.alloc, s.inputs.reinvestRate, 24);
    const bm   = data.findIndex(d=>d.profit>0);
  ...
  const chartData = MONTHS_36.slice(0,24).map((month, i) => {
    const row = { month };
...
export default function App() {
  ...
  const [allocMeta,    setAllocMeta]    = useState(DEFAULT_ALLOC_META);
  ...
  // When allocMeta changes, ensure alloc has all keys (add missing with 0, but normalize)
  const onAllocMeta = useCallback((newMeta) => {
    setAllocMeta(newMeta);
    setAlloc(prev => {
      const next = { ...prev };
      newMeta.forEach(m => { if (next[m.key] === undefined) next[m.key] = 0; });
      return next;
    });
  }, []);
  ...
  const onLoadScenario = useCallback((scenario) => {
    ...
    setAllocMeta(DEFAULT_ALLOC_META);
    setReinvestRate(scenario.inputs.reinvestRate);
    setTab("overview");
  }, []);

  const data   = useMemo(() => buildFinancials(inputs.investment, inputs.sellPrice, inputs.productCost, alloc, reinvestRate, numMonths), [inputs, alloc, reinvestRate, numMonths]);
  const budget = useMemo(() => buildBudget(inputs.investment, alloc, allocMeta), [inputs.investment, alloc, allocMeta]);
  const bm     = data.findIndex(d => d.profit > 0);
    ...
    <div style={{ minHeight:"100vh", background:C.dark, color:C.cream, fontFamily:sans }}>
      ...
      <div style={{ background:C.header, borderBottom:`1px solid ${C.gold}33`, padding:"16px 32px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          ...
          <div style={{ color:C.muted, fontSize:11, fontFamily:mono, marginTop:2 }}>B2C · China Sourcing · Riyadh · {numMonths}-Month Model · Tiered Reinvestment</div>
        </div>
      ...
      <div style={{ padding:"26px 32px", maxWidth:1200, margin:"0 auto" }}>
        {tab==="overview"  && <OverviewTab    data={data} budget={budget} inputs={inputs} alloc={alloc} onAlloc={setAlloc} allocMeta={allocMeta} onAllocMeta={onAllocMeta}/>}
        ...
        {tab==="timeline"  && <TimelineTab numMonths={numMonths} onNumMonths={setNumMonths}/>}
